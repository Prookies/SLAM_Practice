...
int main(int argc, char **argv) {
  ...
  // ----------  开始你的代码  --------------
  // 计算原图四个角点和目标区域对应角点的Homography
  if (data.points.size() == 4) {
    Mat h = findHomography(pts_src, data.points);
    // 用H对原图做变换，warpPerspective是透视变换
    warpPerspective(im_src, im_temp, h, im_temp.size());
    imshow("warp", im_temp);
    // 提取鼠标点击的四个角点
    Point pts_u[4];
    for (size_t i = 0; i < 4; i++) {
      pts_u[i] = data.points[i];
    }
    // 把目标图中对应区域像素值设置为0，fillConvexPoly是绘图函数
    fillConvexPoly(im_dst, pts_u, 4, Scalar(0), CV_AA);
    imshow("fill", im_dst);
