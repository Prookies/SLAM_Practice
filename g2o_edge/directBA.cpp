/****************************
 * 题目：用直接法Bundle Adjustment
*估计相机位姿，已经给定3张图，poses.txt,points.txt文件。
 *
 * 本程序学习目标：
 * 熟悉g2o库顶点和边的定义。
 *
...
// 分别对应VertexSBAPointXYZ和VertexSophus顶点，观测值为16维
typedef Eigen::Matrix<double, 16, 1> Vector16d;
class EdgeDirectProjection
    : public g2o::BaseBinaryEdge<16, Vector16d, g2o::VertexSBAPointXYZ,
                                 VertexSophus> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  ...
  virtual void computeError() override {
    ...
    // ----------------  开始你的代码 ----------------------//
    // 参考十四讲中直接法BA部分
    //    const double dx[4] = {-2, -1, 0, 1};
    //    const double dy[4] = {-2, -1, 0, 1};
    // 李群相机位姿v1
    ...
    // 首先进行坐标转换
    //    Vector3D P(v2->estimate());
    //    Vector4D P_h = Vector4D(P[0], P[1], P[2], 1.0);
    //    Matrix4D T = v1->estimate().matrix();
    //    Matrix3d R = T.block(0, 0, 3, 3);
    //    Vector3D t = T.block(0, 3, 3, 1);
    // 这里输入的是3维向量

    Vector3d Pc = v1->estimate() * v2->estimate();
    float u = Pc[0] / Pc[2] * fx + cx;
    float v = Pc[1] / Pc[2] * fy + cy;
    //    Vector2d Puv = cam->cam_map(Pc);

    if (bInImage(u - 10, v - 10, w, h) && bInImage(u + 10, v + 10, w, h)) {
      ...
      for (int i = -2; i < 2; i++) {
        for (int j = -2; j < 2; j++) {
          _error[k] = origColor[k] - GetPixelValue(targetImg, u + j, v + i);
        }
      }

    } else {
      this->setLevel(1); // 很重要，设置为野值边
  ...
  virtual void linearizeOplus() override {
    ...
    const VertexSBAPointXYZ *vertexPw =
        static_cast<const VertexSBAPointXYZ *>(vertex(0));
    const VertexSophus *vertexTcw =
        static_cast<const VertexSophus *>(vertex(1));
    // 相机坐标系下空间点的坐标
    ...
    for (int i = -2; i < 2; i++)
      for (int j = -2; j < 2; j++) {
        //
        int num = 4 * i + j + 10;
        // 像素梯度，使用的方法为中值差分，即前后做差除以2
        J_I_Puv(0, 0) = (GetPixelValue(targetImg, u + i + 1, v + j) -
                         GetPixelValue(targetImg, u + i - 1, v + j)) /
                        2;
        J_I_Puv(0, 1) = (GetPixelValue(targetImg, u + i, v + j + 1) -
                         GetPixelValue(targetImg, u + i, v + j - 1)) /
                        2;
        _jacobianOplusXi.block<1, 3>(num, 0) =
            -J_I_Puv * J_Puv_Pc * vertexTcw->estimate().rotation_matrix();
...
int main(int argc, char **argv) {
  ...
  cout << "poses: " << poses.size() << ", points: " << points.size()
       << ", color: " << color.size() << endl;
  ...
  //    // old g2o version
  //    typedef g2o::BlockSolver< g2o::BlockSolverTraits<6,3> > Block;  // pose
  //    维度为 6, landmark 维度为 3
  ...
  index = 0;
  for (auto pose : poses) {
    VertexSophus *v1 = new VertexSophus();
    v1->setId(num_point + index);
    v1->setEstimate(pose);
    optimizer.addVertex(v1);
    index++;
  }
  int num_pose = poses.size();

  Vector2d principle_point{cx, cy};

  g2o::CameraParameters *camera =
      new g2o::CameraParameters(fx, principle_point, 0.);
  camera->setId(0);
  ...
  for (int i = 0; i < num_pose; i++) {
    for (int j = 0; j < num_point; j++) {
      EdgeDirectProjection *edge =
          new EdgeDirectProjection(color[j], images[i]);
      edge->setVertex(
          0, dynamic_cast<g2o::VertexSBAPointXYZ *>(optimizer.vertex(j)));
      edge->setVertex(
          1, dynamic_cast<VertexSophus *>(optimizer.vertex(i + num_point)));
      // 信息矩阵的维度和误差的维度的关系
      edge->setParameterId(0, 0); // 如何设置
  ...
  for (int c = 0; c < poses.size(); c++) {
    for (int p = 0; p < points.size(); p++) {
      Vector3d Pw =
          dynamic_cast<VertexSBAPointXYZ *>(optimizer.vertex(p))->estimate();
      points[p] = Pw;
    }
    SE3 Tcw = dynamic_cast<VertexSophus *>(optimizer.vertex(c + points.size()))
                  ->estimate();
    poses[c] = Tcw;
  }
...
void Draw(const VecSE3 &poses, const VecVec3d &points) {
  ...
  cout << "Draw poses: " << poses.size() << ", points: " << points.size()
       << endl;
